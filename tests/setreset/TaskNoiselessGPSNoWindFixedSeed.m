classdef TaskNoiselessGPSNoWindFixedSeed<Task
    % Task used to test assertions on DT
    %
    methods (Sealed,Access=public)
                        
        function obj = TaskNoiselessGPSNoWindFixedSeed(state)
            obj = obj@Task(state);
        end
        ...
        function taskparams=init(~)
            ...
            taskparams.platforms(1).configfile = 'pelican_config_noiseless_GPS_no_turbulence';
