classdef Pelican<Steppable & Platform
    ...
    methods (Sealed,Access=protected)
        ...
        function US = scaleControls(obj,U)
            ...
            % bat [9..12] Volts battery voltage
            %
            assert(size(U,1)==5,'pelican:inputoob',['wrong size of control inputs ',num2str(size(U,1)),' instead of 5\n',...
                '\tU = [pt;rl;th;ya;bat] \n']);
            
            for i = 1:size(U,2),
            assert(all(U(:,i)>=obj.CONTROL_LIMITS(:,1)) && all(U(:,i)<=obj.CONTROL_LIMITS(:,2)),...
                'pelican:inputoob',['control inputs values not within limits ',num2str(U(:,i)'),'\n',...
                '\tU = [pt;rl;th;ya;bat] \n\n',...
