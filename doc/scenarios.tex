...
Under the assumptions of constant wind (with magnitude $u$), a source with constant emission rate $Q$, a sufficiently long time scale of interest, isotropic diffusion, negligible ground penetration and negligible variations in topography; a closed form solution for the plume concentration can be obtained (\cite{stockie2011}).
The resulting concentration has the shape of a cone starting at the plume source and expanding downwind. Profile concentrations are symmetric about the plume centerline and have the shape of a Gaussian(see figure \ref{fig:winddisp}).
\begin{figure}[ht]
...
In the case of multiple ($S$) sources the total concentration can be computed by superposition of the effects of each of the single source:
\begin{align}\label{eqn:multiplesourcesgaussiandispersion}
c(x',y',z) = \sum_{s=1}^S c(x',y',z;X'_s,Y'_s,H_s,Q_s).
\end{align}
Where we assume that the wind acting on each source is the same and therefore the wind frame of reference is unique.
