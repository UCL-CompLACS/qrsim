...
The classifier model needs to be queried multiple times for each of the simulated observation, so it is important to limit its computational complexity; at this aim at present we only focus on modelling the dependency between altitude and the score returned by the classifier.
Altitude directly effects the size that a person in the field of view has on the camera image, therefore is the primary variable that effects the classifier ability to perform detections.
