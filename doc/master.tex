...
The second of the mathods that is necessary to define in a task is the method \texttt{reset()}.
...
\end{lstlisting}
In such method\footnote{Prior to version 1.2.0 the syntax \texttt{taskparams.platforms(1).X} was used within the init method to set the platform initial state, such syntax is now meaningless.} the user specifies that starting state of each platform and also of any other task specific objects.
